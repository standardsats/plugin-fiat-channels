...
case class LastCrossSignedState(
  ...
  def stateUpdate: StateUpdate =
    StateUpdate(blockDay, localUpdates, remoteUpdates, rate, localSigOfRemote)
...
case class ProposeInvoice(description: String, invoice: Bolt11Invoice)
    extends HostedChannelMessage
...
case class MarginChannel(
    newCapacity: Satoshi,
    newRate: MilliSatoshi,
    clientSig: ByteVector64 = ByteVector64.Zeroes
) extends HostedChannelMessage {
  def isRemoteMargined(remote: LastCrossSignedState): Boolean =
    newCapacity.toMilliSatoshi == remote.initHostedChannel.channelCapacityMsat && newRate == remote.rate
  def sign(priv: PrivateKey): MarginChannel = MarginChannel(
    clientSig = Crypto.sign(Crypto.sha256(sigMaterial), priv),
    newCapacity = newCapacity,
    newRate = newRate
  )
  def verifyClientSig(pubKey: PublicKey): Boolean =
    Crypto.verifySignature(Crypto.sha256(sigMaterial), clientSig, pubKey)
  def newLocalBalance(state: LastCrossSignedState): MilliSatoshi = MilliSatoshi(
    (state.rate.toLong.toDouble * state.localBalanceMsat.toLong.toDouble / newRate.toLong.toDouble).round
  )
  def newRemoteBalance(state: LastCrossSignedState): MilliSatoshi = MilliSatoshi(
    (newRate.toLong.toDouble * state.remoteBalanceMsat.toLong.toDouble / state.rate.toLong.toDouble).round
  )
  lazy val sigMaterial: ByteVector = Protocol.writeUInt64(
    newCapacity.toLong,
    ByteOrder.LITTLE_ENDIAN
  ) ++ Protocol.writeUInt64(newRate.toLong, ByteOrder.LITTLE_ENDIAN)
  lazy val newCapacityMsatU64: UInt64 = UInt64(
    newCapacity.toMilliSatoshi.toLong
  )
}
